...
exports.mainPubSub = (event, context, callback) => {
  ...
  const parsedEvents = message.split(newlinePattern);

  zlib.gzip(JSON.stringify(
    parsedEvents.filter((logEvent) => logEvent.trim().length > 0).filter((logEvent, index) => index % sampling == 0).map((logEvent) => {
            try {
            ...
            return {
                ...
                "timestamp": Date.now(context.timestamp),
                "severity": severity,
                "text": logEvent,
                "threadId": context.resource.name
            };
        })
